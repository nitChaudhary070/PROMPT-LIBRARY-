...
export default function Login() {
  ...
  const [mode, setMode] = useState<'select' | 'email' | 'email-signup'>('select');
      ...
      <div className="max-w-md w-full space-y-8 bg-white dark:bg-zinc-900 p-10 rounded-3xl shadow-xl transition-colors duration-300">
        <div>
          ...
          <h2 className="text-center text-3xl font-extrabold text-gray-900 dark:text-white transition-colors">
            {mode === 'select' && 'Sign in to your account'}
            {mode === 'email' && 'Sign in with Email'}
            {mode === 'email-signup' && 'Create an Account'}
          </h2>
        </div>
        ...
        <div className="mt-8 space-y-6">
          ...
          {mode === 'select' && (
            <div className="space-y-4">
              ...
              <button
                ...
                className="group relative w-full flex justify-center py-3 px-4 border border-gray-300 dark:border-zinc-700 text-sm font-medium rounded-xl text-gray-900 dark:text-white bg-white dark:bg-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black dark:focus:ring-offset-zinc-950 transition-all"
              >
                Continue with Email
              </button>
            </div>
          ...
          {(mode === 'email' || mode === 'email-signup') && (
            <form onSubmit={mode === 'email' ? handleEmailLogin : handleEmailSignup} className="space-y-4">
              <div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-sm text-sm font-medium text-white dark:text-black bg-black dark:bg-white hover:bg-gray-800 dark:hover:bg-zinc-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black dark:focus:ring-offset-zinc-950 transition-all disabled:opacity-50"
              >
                {loading ? 'Please wait...' : (mode === 'email' ? 'Sign In' : 'Sign Up')}
              </button>
              
              <div className="flex justify-between text-sm mt-4">
                <button type="button" onClick={() => setMode('select')} className="text-gray-500 hover:text-black dark:hover:text-white transition-colors">
                  Back
                </button>
                <button 
                  type="button" 
                  onClick={() => setMode(mode === 'email' ? 'email-signup' : 'email')} 
                  className="text-blue-600 dark:text-blue-400 hover:underline"
                >
                  {mode === 'email' ? 'Create an account' : 'Already have an account?'}
                </button>
              </div>
            </form>
          )}
